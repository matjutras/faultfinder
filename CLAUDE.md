...
4. Second and third device through the importer only, zero app-code changes — this is the check that the pipeline actually generalizes — done (`bridge_rectifier_06`/`simple_led_07`; further confirmed against another real board, `reverse_polarity_08`, which is what surfaced most of the gotchas below)
