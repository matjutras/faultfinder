...
- Importing `reverse_polarity_08` (a real board authored in KiCad 10) surfaced that `kicad-cli`/`pcbnew` 9.0.8 cannot parse a KiCad 10 file at all — not a version-number guard, genuinely different syntax (confirmed by hand-patching the version/generator header down to a KiCad 9 value and re-running; still failed). Fixed at the time by upgrading the system's KiCad install to 10.0.6 via `ppa:kicad/kicad-10.0-releases` (note the `-releases` suffix — `ppa:kicad/kicad-10.0` alone doesn't exist on Launchpad). That upgrade changed one real, confirmed pcbnew output detail: a `.kicad_pcb` segment's net code is now written quoted (`(net "0")`) where KiCad 9 wrote it bare (`(net 3)`) — `test_build_pcb.py`'s `_SEGMENT_RE` was updated to accept either; this is not a routing regression, the crossing-detection logic itself still finds zero same-layer crossings on every hand-authored device after the upgrade. **This is now also a real, permanent, automated check, not just a note to remember**: `kicad_import.check_kicad_version_compatible` (called from `export_netlist_xml`, i.e. every device's `/import`) and its duplicate in `scripts/import_real_pcb.py` (called before `pcbnew.LoadBoard`, since that's what actually crashes on a too-new file) compare a source file's own `(generator_version "X.Y")` header against `kicad-cli version`'s installed version and raise/exit with a clear message — `"<file> was authored in KiCad 10.0, but only KiCad 9.0.8 is installed..."` — instead of a bare parse crash. If importing a future real board fails with that message, upgrade the system KiCad install (as above); if it fails to parse with a *different* error despite passing this check, that's a genuinely new incompatibility, not this one.
...
- A real board's ref designators can collide with SPICE's *reserved* element-prefix letters outright, not just this app's own ref-prefix type table above — `battery_protection_09`'s fuse is ref "F1", but SPICE's bare "F" prefix means a current-controlled current source (a 4-argument dependent source), so `circuit.cir` had to rename it to `R4` (it's modeled as a plain resistor) and its LED (ref "LED1") to `D2` (SPICE's bare "L" prefix means inductor, which "LED1 ... LEDMOD" isn't). `circuit.cir`'s own component names have always been independent of the schematic's real refs (same reason every device's source is always `V1`); `map.json`'s probe geometry is unaffected since it comes from `kicad_import.py` parsing the `.kicad_sch` directly. **This is now also a real, permanent, automated check, not just a note to remember**: `fault_gen.validate_circuit_refs` (called from `generate_fault_pool`, i.e. every device's `/import`) rejects, with a clear message naming the actual collision, any ref whose first letter is one of SPICE's reserved-but-unmodeled prefixes (`RESERVED_SPICE_PREFIXES`: `B E F G H J K O S T U W X Z`) or one of the modeled passive prefixes (`R`/`L`/`C`) used without the trailing numeric value that prefix requires — the second case is what actually catches an "LED1"-style collision, since `L` itself is a legitimately modeled prefix and only the missing numeric value gives away that this line isn't really an inductor. If a future real board's `circuit.cir` throws this error (or an ngspice syntax error mentioning "not enough parameters" for a component whose SPICE line otherwise looks right, meaning something slipped past the check), rename the offending ref the same way, and consider whether the check itself needs to widen.
